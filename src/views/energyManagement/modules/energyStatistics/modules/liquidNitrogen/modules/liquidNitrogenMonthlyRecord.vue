              ...
              <div id="printContent">
                ...
                <table class="table_form" width="100%" border="2">
                  ...
                  <tr style="height: 25px">
                    <td width="15%">序号</td>
                    <td width="25%">日期</td>
                    <td width="25%">消耗量(t)</td>
                    <td width="25%">年累计量(t)</td>
                    <td width="10%">备注</td>
                  </tr>
                  <tr
                  ...
                  >
                    ...
                    <td>{{ item.yearTotNum }}</td>
                    <td>{{ item.remark }}</td>
                  </tr>
                  <tr style="height: 30px">
                    <td>合计</td>
                    <td></td>
                    <td>{{ reportData.consumption }}</td>
                    <td>{{ reportData.yearCumulative }}</td>
                    <td></td>
                  </tr>
                </table>
              </div>
...
import {
  ...
  submit,
  approval,
} from "@/api/energyManagement/energyStatistics/liquidNitrogenMonthlyRecord.js";
  ...
  methods: {
    ...
    getReportDateRangeText(startDate, endDate) {
      if (startDate && endDate) {
        ...
        this.reportData.reportDateRangeText = startDate + " - " + endDate;
    ...
    getDailyList(beginDate, endDate, type) {
      ...
      queryDailyData(searchForm).then((res) => {
        if (res.body) {
          this.dailyDataSource = res.body.dailyDataDtoList;
          //type不为空是编辑页面请求，不需要渲染合计行
          if (!type) {
            //渲染合计行数据
            this.reportData.consumption = res.body.consumption;
            this.reportData.yearCumulative = res.body.yearCumulative;
          }
        } else {
          delete this.reportData.consumption;
          delete this.reportData.yearCumulative;
        }
      });
    },
    ...
    addClick() {
      this.pageType = "add";
      this.modelConfig.title = "液氮消耗月报-新增";
      //生成编号
      let reportCode = "YDXHYB" + this.formatDate(new Date(), "yyyyMMddHHmmss");
    ...
    editClick(val) {
      this.pageType = "edit";
      this.modelConfig.title = "液氮消耗月报-编辑";
      this.visible = true;
      this.reportData = {
        id: val.id,
        //报表年YYYY
        ...
        //年度累计量 合计
        yearCumulative: val.yearCumulative,
        //报表日期范围文字
    ...
    lookClick(val) {
      this.pageType = "look";
      this.modelConfig.title = "液氮消耗月报-详情";
      this.visible = true;
      this.reportData = {
        id: val.id,
        //报表年YYYY
    ...
    getSum() {
      ...
      this.dataSource.forEach((item, index) => {
        item.index =
          ...
          1;
        //累加
        consumption = this.accAdd(
          consumption,
          isNaN(new Number(item.consumption)) ? 0 : new Number(item.consumption)
        );
        // //直接加会失真
        // consumption += isNaN(new Number(item.consumption))
        //   ? 0
        //   : new Number(item.consumption);
      });
      let sumData = {
        id: "-999",
        index: "本页合计",
