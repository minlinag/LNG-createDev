              ...
              <div id="printContent">
                ...
                <table class="table_form" width="100%" border="2">
                  ...
                  <tr style="height: 25px">
                    <td width="10%">日期</td>
                    <td width="18%">发电机柴油消耗量(kg)</td>
                    <td width="18%">消防水泵柴油消耗量(kg)</td>
                    <td width="18%">其他消耗量(kg)</td>
                    <td width="18%">合计消耗量(kg)</td>
                    <td width="18%">备注</td>
                  </tr>
                  <tr
                    class="inputBorderless"
                  ...
                  >
                    ...
                    <td>
                      <a-input
                        ...
                        v-model="dailyDataSource[index].engineConsumption"
                        @change="calculateTotal('engineConsumption', index)"
                      />
                      <span v-else>{{ item.engineConsumption }} </span>
                    </td>
                    <td>
                      <a-input
                        v-if="pageType == 'add' || pageType == 'edit'"
                        v-model="dailyDataSource[index].waterPumpConsumption"
                        @change="calculateTotal('waterPumpConsumption', index)"
                      />
                      <span v-else>{{ item.waterPumpConsumption }} </span>
                    </td>
                    <td>
                      <a-input
                        v-if="pageType == 'add' || pageType == 'edit'"
                        v-model="dailyDataSource[index].otherConsumption"
                        @change="calculateTotal('otherConsumption', index)"
                      />
                      <span v-else>{{ item.otherConsumption }} </span>
                    </td>
                    <td>
                    ...
                    <td>
                      <a-input
                        v-if="pageType == 'add' || pageType == 'edit'"
                        v-model="dailyDataSource[index].remark"
                      />
                  ...
                  <tr style="height: 30px">
                    <td>本月合计</td>
                    <td>{{ reportData.engineConsumption }}</td>
                    <td>{{ reportData.waterPumpConsumption }}</td>
                    <td>{{ reportData.otherConsumption }}</td>
...
import {
  ...
  approval,
  getDetailById,
} from "@/api/energyManagement/energyStatistics/dieselOilMonthlyReport.js";
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          title: "发电机柴油消耗量(kg)",
          dataIndex: "engineConsumption",
          width: 120,
          align: "center",
        },
        {
          title: "消防水泵柴油消耗量(kg)",
          dataIndex: "waterPumpConsumption",
          width: 140,
          align: "center",
        },
        {
  ...
  methods: {
    ...
    getReportDate() {
      ...
      if (year && month) {
        ...
        //根据开始时间结束时间生成数据
        this.getDailyListByBeginDateAndEndDate(beginDate, endDate);
      }
    },

    //根据开始日期  结束日期  生成日报数据
    getDailyListByBeginDateAndEndDate(beginDate, endDate) {
      let newdailyDataSource = [];
      beginDate = new Date(beginDate);
      endDate = new Date(endDate);
      while (endDate.getTime() - beginDate.getTime() >= 0) {
        let year = beginDate.getFullYear();
        let month =
          beginDate.getMonth() + 1 < 10
            ? "0" + (beginDate.getMonth() + 1)
            : beginDate.getMonth() + 1;
        let day =
          beginDate.getDate().toString().length == 1
            ? "0" + beginDate.getDate()
            : beginDate.getDate();
        let date = year + "-" + month + "-" + day;
        let itNotExist = true;
        //判断已存在 复制过来
        this.dailyDataSource.forEach((data) => {
          if (data.reportDate == date) {
            newdailyDataSource.push(data);
            itNotExist = false;
          }
        });
        if (itNotExist) {
          newdailyDataSource.push({
            reportDate: date,
            deleteFlg: 0,
            engineConsumption: "0",
            waterPumpConsumption: "0",
      ...
      this.dailyDataSource = newdailyDataSource;
      let engineConsumptionSum = 0;
      let waterPumpConsumptionSum = 0;
      let otherConsumptionSum = 0;
      let totalConsumptionSum = 0;

      this.dailyDataSource.forEach((element) => {
        //计算列
        engineConsumptionSum = this.accAdd(
          engineConsumptionSum,
          isNaN(new Number(element.engineConsumption))
            ? 0
            : new Number(element.engineConsumption)
        );
        waterPumpConsumptionSum = this.accAdd(
          waterPumpConsumptionSum,
          isNaN(new Number(element.waterPumpConsumptionSum))
            ? 0
            : new Number(element.waterPumpConsumptionSum)
        );
        otherConsumptionSum = this.accAdd(
          otherConsumptionSum,
          isNaN(new Number(element.otherConsumptionSum))
            ? 0
            : new Number(element.otherConsumptionSum)
        );
        totalConsumptionSum = this.accAdd(
          totalConsumptionSum,
          isNaN(new Number(element.totalConsumptionSum))
            ? 0
            : new Number(element.totalConsumptionSum)
        );
      });
      this.reportData.engineConsumption = String(engineConsumptionSum);
      this.reportData.waterPumpConsumption = String(waterPumpConsumptionSum);
      this.reportData.otherConsumption = String(otherConsumptionSum);
      this.reportData.totalConsumption = String(totalConsumptionSum);
    },
    ...
    reportDateChange(dates) {
      if (dates.length != 0) {
         ...
         * 2、生成日报数据，渲染表格中
         */
        this.getDailyListByBeginDateAndEndDate(
    ...
    //当改变值时  计算合计
    calculateTotal(columnName, index) {
      this.dailyDataSource[index].totalConsumption = String(
        this.accAdd(
          this.accAdd(
            isNaN(new Number(this.dailyDataSource[index].engineConsumption))
              ? 0
              : new Number(this.dailyDataSource[index].engineConsumption),
            isNaN(new Number(this.dailyDataSource[index].waterPumpConsumption))
              ? 0
              : new Number(this.dailyDataSource[index].waterPumpConsumption)
          ),
          isNaN(new Number(this.dailyDataSource[index].otherConsumption))
            ? 0
            : new Number(this.dailyDataSource[index].otherConsumption)
        )
      );
      //计算列
      let sum = 0;
      this.dailyDataSource.forEach((item) => {
        sum = this.accAdd(
          sum,
          isNaN(new Number(item[columnName])) ? 0 : new Number(item[columnName])
        );
      });
      this.reportData[columnName] = String(sum);
      this.reportData.totalConsumption = this.accAdd(
        this.accAdd(
          isNaN(new Number(this.reportData.engineConsumption))
            ? 0
            : new Number(this.reportData.engineConsumption),
          isNaN(new Number(this.reportData.waterPumpConsumption))
            ? 0
            : new Number(this.reportData.waterPumpConsumption)
        ),
        isNaN(new Number(this.reportData.otherConsumption))
          ? 0
          : new Number(this.reportData.otherConsumption)
      );
    },
    ...
    addClick() {
      this.pageType = "add";
      this.modelConfig.title = "柴油消耗统计表-新增";
      //生成编号
      let reportCode = "CYXHYB" + this.formatDate(new Date(), "yyyyMMddHHmmss");
    ...
    editClick(val) {
      this.pageType = "edit";
      this.modelConfig.title = "柴油消耗月报-编辑";
      this.visible = true;
      this.reportData = {
        id: val.id,
        //报表年YYYY
        ...
        //发电机柴油消耗量合计
        engineConsumption: val.engineConsumption,
        //消防水泵柴油消耗量合计
        waterPumpConsumption: val.waterPumpConsumption,
        //其他消耗量合计
        otherConsumption: val.otherConsumption,
        //总计
      ...
      //查询子表数据
      this.getDetailsListById(val.id);
    },

    //根据主表编号  查询 详情信息
    getDetailsListById(id) {
      getDetailById(id).then((res) => {
        if (res.code == "200") {
          this.dailyDataSource = res.body;
        } else {
          this.$message.warning("获取详情信息失败");
        }
      });
    },
    ...
    lookClick(val) {
      this.pageType = "look";
      this.modelConfig.title = "柴油消耗月报-详情";
      this.visible = true;
      this.reportData = {
        id: val.id,
        //报表年YYYY
    ...
    saveOperation(type) {
      this.reportData.dieselOilMonthlyReportSubList = this.dailyDataSource;
    ...
    // 合计
    getSum() {
      let engineConsumption = 0;
      let waterPumpConsumption = 0;
      ...
      this.dataSource.forEach((item, index) => {
        item.index =
          ...
          1;
        //发电机柴油消耗量
        engineConsumption = this.accAdd(
          engineConsumption,
          isNaN(new Number(item.engineConsumption))
            ? 0
            : new Number(item.engineConsumption)
        );
        //发电机柴油消耗量
        waterPumpConsumption = this.accAdd(
          waterPumpConsumption,
          isNaN(new Number(item.waterPumpConsumption))
            ? 0
            : new Number(item.waterPumpConsumption)
        );
        //其他消耗量
        otherConsumption = this.accAdd(
          otherConsumption,
          isNaN(new Number(item.otherConsumption))
            ? 0
            : new Number(item.otherConsumption)
        );
        //合计消耗量
        ...
        //直接加会失真 弃用
        // engineConsumption += isNaN(new Number(item.engineConsumption))
        //   ? 0
        //   : new Number(item.engineConsumption);
        // waterPumpConsumption += isNaN(new Number(item.waterPumpConsumption))
        //   ? 0
        //   : new Number(item.waterPumpConsumption);
        // otherConsumption += isNaN(new Number(item.otherConsumption))
        //   ? 0
        //   : new Number(item.otherConsumption);
      ...
      let sumData = {
        ...
        waterPumpConsumption: waterPumpConsumption,
