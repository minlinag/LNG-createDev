<template>
  <Breadcrumb>
    ...
    <template slot="appMain">
      <div style="position: relative;" class="position-scrList">
      ...
      >
        ...
        <template slot="operation" slot-scope="row">
          <pe-button
            ...
            @approval="
              viewJump(row, { page: 'scr', type: 'edit' }, '设备报废-审批')
            "
            @viewJump="
              viewJump(row, { page: 'scr', type: 'view' }, '设备报废-详情')
            "
          ></pe-button>
        </template>
      </dc-table>
...
<script>
import formModel from '../equipmentDismantleMgt/components/formModel.vue'
import detailModel from '../equipmentDismantleMgt/components/detailModel.vue'
import {
  getList,
  deleteData
...
export default {
  name: 'equipmentScrap',
  components: { formModel, detailModel },
  filters: {
    recordStatus(val) {
      let value = ''
      switch (val) {
  ...
  data() {
    return {
      ...
      // 添加弹窗控制
      visible: false,
      typeModel: { page: 'scr', type: 'add' },
      title: '',
      // 派工弹窗控制
      dispatchVisible: false,
      // 详情弹窗控制
      detailVisible: false,
      detailTypeModel: { page: 'scr', type: 'view' },
      detailTitle: '',
      // 状态下拉框
      recordStatusOptions: [
        {
  ...
  methods: {
    // 下拉框输入框和表单分离问题
    getPopupContainer(){
      return document.querySelector('.position-scrList')
    },
    init() {
      this.getList()
      this.dictList()
    ...
    // 新增
    addMonth() {
      this.typeModel = { page: 'scr', type: 'add' }
      this.title = '新增设备报废'
      this.visible = true
      this.$nextTick(() => {
    ...
    // 编辑
    editMonth(row) {
      this.typeModel = { page: 'scr', type: 'edit' }
      this.title = '编辑设备报废'
      this.visible = true
      this.$nextTick(() => {
