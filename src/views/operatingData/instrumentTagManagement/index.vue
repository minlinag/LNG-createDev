  ...
  <div class="device-type-maintenance">
    <Breadcrumb>
      <template slot="breadcrumbButton">
        <pe-button
          :include="[
            { value: 'Maintain', clickName: 'addListClick' },
...
export default {
  ...
  methods: {
    ...
    editClick(val) {
      this.formTitle = '仪表位号管理-编辑'
      this.getQueryByID(val.id)

    },
    detailsClick(val) {
      this.formTitle = '仪表位号管理-详情'
      this.getQueryByID(val.id)
    },
    getQueryByID(id) {
      queryByID({ id }).then((res) => {
        if (res.code == "200") {
          ...
          let formList = res.body[0]
          if (formList.inspectionItemName) {
            formList.maintenanceCategory = '1'
          } else {
            formList.maintenanceCategory = '2'
          }
          this.maintainLableList = formData
          this.hiddenVisible = true
        }
      })
    },
