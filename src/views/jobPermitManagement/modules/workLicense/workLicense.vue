      ...
      >
        ...
        <template slot="operation" slot-scope="row, text">
          <pe-button
            ...
            @updataSub="updataSub(row, 'query')"
            @updataList="updataList(row, 'copy')"
          ></pe-button>
        </template>
      </dc-table>
      <div v-if="visible">
        <dc-Model
          ...
          class="DcModel"
          v-if="dataForm.type == 'updata' || dataForm.type == 'add'||dataForm.type=='copy'"
        >
          <workLicenseForm
            :dataType="dataForm.type"
            @clear="clear"
        ...
        <dc-Model
          ...
          class="DcModel"
        >
          <infoDetail :dataForm="dataForm.body"> </infoDetail>
        </dc-Model>
      </div>
    </template>
  </Breadcrumb>
...
export default {
  ...
  methods: {
      ...
      updataList(val,type){
          var dataCode=''
          this.modelConfig.title = '作业许可申请'
          this.dataForm.body = null
          this.dataForm = {}
          this.dataForm.type = type
              getJsonByCode({ strCode: val.id }).then((res) => {
                  this.dataForm.body = res.body
                  this.dataForm.body.baseInfo.relationID=null;
                  res.body.baseInfo.createdDateTime='';
                  getCreateNewId({ typeCode: res.body.baseInfo.operationTypeCode }).then(
                      (res) => {
                          if (res.code == '200') {
                              this.dataForm.body.baseInfo.id = res.body
                              this.visible = true
                          }
                      }
                  )
    ...
    gltjAdd() {
      if(this.selectedRowKeys.length){
        ...
        idList: this.selectedRowKeys.toString(),
      }
      addGltj(data).then((res) => {
        if (res.code == '200') {
    ...
    upDateListDetail(val, type) {
      this.modelConfig.title = type=='query'?'作业许可详情':'作业许可编辑'
      this.dataForm.body = null
      this.dataForm = {}
      this.dataForm.type = type
      getJsonByCode({ strCode: val.id }).then((res) => {
    ...
    //获取JAS下拉框列表
    getJasList() {
      jsaList({ pageNum: 1, pageSize: 99999 }).then((res) => {
        this.JSASelect = res.body.data
      })
    },
    closeApply(val, type) {
