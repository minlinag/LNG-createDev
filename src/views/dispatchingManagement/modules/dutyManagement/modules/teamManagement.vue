...
<script>
import {
  TeamGroupfindAllList,
  TeamGroupDeleteBatch,
} from "@/api/dispatchingManagement/teamManagement.js";

import teamManagementDetails from "./teamManagementDetails.vue";
...
const columns = [
  ...
  { title: "班组成员(内部)", ellipsis: true, dataIndex: "teamMember" },
  { title: "班组成员(外部)", ellipsis: true, dataIndex: "externalTeamMember" },
  { title: "创建人", ellipsis: true, dataIndex: "createdUser" },
  { title: "创建时间", ellipsis: true, dataIndex: "createdDateTime" },
  {
    fixed: "right",
    title: "操作",
    width: 90,
...
export default {
  components: { teamManagementDetails },
  data() {
    return {
      formType: "",
      recode: {},
  ...
  methods: {
    ...
    //批量删除确认方法
    deleteListClick(val) {
      TeamGroupDeleteBatch(this.selectedRowKeys).then((res) => {
        console.log(res);
        if (res.code == "200") {
    ...
    //获取表格列表
    getList() {
      TeamGroupfindAllList(this.searchInfo).then((res) => {
