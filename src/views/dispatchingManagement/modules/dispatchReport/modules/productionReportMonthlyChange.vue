  ...
  <Breadcrumb>
    <template slot="appMain">
      ...
      <div>
                  ...
                  >
                    ...
                    >
                      <a-input
                        ...
                        @change="
                          inputChange($event, item.label, item.highLimit)
                        "
                      />
                    </a-form-item>
                  </a-col>
                ...
                <div class="gbnhTableDiv">
                  <table width="100%" class="gbnhTable">
                    ...
                    <tr class="evenTr">
                      ...
                      >
                        <a-input
                          class="evenBackground"
                          v-if="pageType === 'edit' || pageType == 'approval'"
                          v-model="data[`${item.parameterName}`]"
                          @change="
                            inputChange($event, item.label, item.highLimit)
                          "
                        />
                        <span v-else>{{ data[`${item.parameterName}`] }}</span>
                      </td>
                    </tr>
                    <tr class="oddTr">
        ...
        <a-row :gutter="24" class="bottomButton">
          <div style="text-align: center">
            ...
            <a-button
              ...
              >确定</a-button
            >
            <a-button @click="returnClick">返回</a-button>
          </div>
        </a-row>
      </div>
    </template>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          title: "耗电量(kwh)",
          dataIndex: "shiftPowerConsumption",
          slots: { customRender: "shiftPowerConsumption" },
          align: "center",
        },
        {
          title: "耗水量(t)",
          dataIndex: "shiftWaterConsumption",
          slots: { customRender: "shiftWaterConsumption" },
          align: "center",
        },
        {
          title: "耗液氮量(t)",
          dataIndex: "shiftLiquidNitrogenConsumption",
          slots: { customRender: "shiftLiquidNitrogenConsumption" },
          align: "center",
        },
        {
          title: "SCV自耗气(万方)",
          dataIndex: "shiftSCVSinceGasConsumption",
          slots: { customRender: "shiftSCVSinceGasConsumption" },
          align: "center",
        },
        {
          title: "气化总量(万方)",
          dataIndex: "shiftGasificationTotalAmount",
          slots: { customRender: "shiftGasificationTotalAmount" },
          align: "center",
        },
        {
          title: "装车总量(万方)",
          dataIndex: "shiftLoadingTotalAmount",
          slots: { customRender: "shiftLoadingTotalAmount" },
          align: "center",
        },
      ],
      dataSource: [
  ...
  mounted() {
    ...
    if (this.$route.query.id) {
      //根据编号查询详情信息
      queryDetailById(this.$route.query.id).then((res) => {
        if (res.code == 200) {
          this.data = res.body;
      ...
      this.$message.warning("获取数据失败，请返回重试");
    }
  },
  methods: {
    //提交 开启流程
    ...
    //点击保存按钮
    saveClick() {
      this.data.fillUser = this.$store.state.user.name;
      this.data.compilingDepartment = this.$store.state.user.unitName;
    ...
    inputChange(e, label, highLimit) {
      let value = isNaN(Number(e.target.value)) ? 0 : Number(e.target.value);
      if (value > highLimit) {
        this.$message.warning(
          label + "高限为：" + highLimit + ",请检查后重新输入！"
        );
      }
    },
