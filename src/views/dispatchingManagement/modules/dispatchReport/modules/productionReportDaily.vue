    ...
    <template slot="appMain">
      ...
      >
        <template slot="releaseDate" slot-scope="text, row">
          <span class="edit" @click.stop="lookClick(row)">
            {{ text }}
          </span>
        </template>
...
<script>
import { getList } from "@/api/dispatchingManagement/productionReportDaily.js";
      ...
      getList(this.searchForm).then((res) => {
        ...
        this.dataSource.forEach((item) => {
          ...
          this.stateOptions.forEach((state) => {
            if (item.recordStatus == state.value) {
              item.recordStatusName = state.label;
            }
          });
          ...
          if (item.productionBeginDate && item.productionEndDate) {
            item.productionDate =
              this.formatDate(
                new Date(item.productionBeginDate),
                "yyyy-MM-dd HH:mm"
              ) +
              "-" +
              this.formatDate(
                new Date(item.productionEndDate),
                "yyyy-MM-dd HH:mm"
              );
          }
        });
