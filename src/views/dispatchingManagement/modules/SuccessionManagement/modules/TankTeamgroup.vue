      ...
      <pe-button
        :include="[
          ...
          { value: 'Delete', clickName: 'deleteListClick' },
          {
            ...
            isPower: false,
            url: '/tankerArea/exportOperationCardVersion',
          },
        ]"
        @addNew="addNew"
...
import TankTeamgroupDetails from "./TankTeamgroupDetails.vue";
...
export default {
  components: { TankTeamgroupDetails },
  data() {
    return {
      formType: "",
      recode: {},
  ...
  methods: {
    ...
    //批量删除确认方法
    deleteListClick(val) {
      tankerAreadelete(this.selectedRowKeys).then((res) => {
        if (res.code == "200") {
    ...
    //获取表格列表
    getList() {
      tankerAreaselect(this.searchInfo).then((res) => {
