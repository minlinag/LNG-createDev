...
import {
  personnelExamineQueryPostName,
  personnelExamineSelectPostDutyData, personnelExamineInsert,
  personnelExamineInsertAssess,
  personnelExamineUpdateAssess,
  personnelExamineQueryByID,
  personnelExamineQueryAssessByID,
  personnelExamineDeleteBatchAssess
} from "@/api/HSESystemFileManagement/departmentalResponsibilitySystemAssessment"
export default {
  name: "departmentalResponsibilitySystemAssessmentAddEditForm",
    ...
    getExamineQueryByID() {
      personnelExamineQueryByID({ id: this.formModel.id }).then((res) => {
        if (res.code == '200') {
          this.$nextTick(() => {
            this.$set(this.formModel, 'list', res.body[1])
            this.formModel = this.$lodash.cloneDeep(this.formModel);
          })
