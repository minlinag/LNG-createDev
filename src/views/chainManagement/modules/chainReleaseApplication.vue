...
export default {
  ...
  methods: {
    ...
    toBeImplemented(val, callback) {
      if (val.executorTime) {
        var applyTime = +new Date(val.createdDateTime);
        var executorTime = +new Date(val.executorTime);
        if (applyTime * 1 > executorTime * 1) {
          return this.$message.warning("执行时间要晚于申请时间");
        }
      }
    ...
    submitForm(val, stateMonitor, action) {
      var startSj = +new Date(val.planRelieveTime);
      var endSj = +new Date(val.planRecoverTime);
      if (startSj * 1 > endSj * 1) {
        return this.$message.warning("计划解除时间不能晚于计划恢复时间");
      }
      if (val.id) {
    ...
    getListDetails(value, position, callback) {
      queryByID({ id: value.id }).then((res) => {
        ...
        this.$refs.addReleaseFormRef.addReleaseForm.whetherExecutor = "1";
        if (
    ...
    addClick() {
      this.addReleaseTitle = "联锁解除申请-新增";
      this.typeModel = "add";
