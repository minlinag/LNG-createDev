...
import { professionHealthArchivesFindAllList, professionHealthArchivesQueryByID, professionHealthArchivesDeleteBatch } from "@/api/occupationalHealthManagement/occupationalHealthArchivesManagement"
  ...
  methods: {
    ...
    deleteClick() {
      if (this.selectedRowKeys.length > 0) {
        professionHealthArchivesDeleteBatch(this.selectedRowKeys).then((res) => {
          if (res.code == '200') {
    ...
    getList() {
      professionHealthArchivesFindAllList(this.searchField).then((res) => {
    ...
    // },
    addClick() {
      this.formTitle = "职业健康档案-新增";
      this.checkForm = false;
      this.isAssessment = false
      this.hiddenVisible = true;
    },
    editClick(row) {
      this.formTitle = "职业健康档案-编辑";
      this.checkForm = false;
      this.isAssessment = false
      this.hiddenVisible = true;
      this.$nextTick(() => {
    ...
    assessmentClick(row) {
      this.formTitle = "职业健康档案-体检";
      this.checkForm = false;
      this.isAssessment = true
      this.hiddenVisible = true;
      this.$nextTick(() => {
        professionHealthArchivesQueryByID({ id: row.id }).then((res) => {
          if (res.code == '200') {
            let newRow = row
            newRow.list = res.body[3].map((item) => {
              let newItem = item
              newItem.fileID = this.$fileUploadJSON.toParse(newItem.fileID)
              return newItem
            })
            newRow.harmFactor = res.body[1].harmFactor
            this.$refs.addEditFormRef.formModel = newRow
            this.$refs.addEditFormRef.physicalExaminationTableList = res.body[2]
          }
        });
      })
