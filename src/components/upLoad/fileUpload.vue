<template>
  <div class="clearfix">
    ...
    <div class="file_list" v-if="fileBack">
      <div
        class="file_content"
    ...
    <pdf-preview
      ...
      v-if="fileTypeMIME === 'application/pdf' && previewShow"
    ></pdf-preview>
    <image-previewer
      :img-show="previewShow"
      :img-url="previewUrl"
      @imgCancel="previewCancel"
      v-if="
...
export default {
  ...
  methods: {
    ...
    downloadFileClick(item) {
      //从文件名中截取后缀名
      let type = item.fileName.match(/[^.]+$/)[0];
      if (item.fileType || type) {
        //fileType不为空时，使用fileType；为空时，使用文件名中截取的后缀名
        let fileType = item.fileType ? item.fileType.substr(1) : type;
        // fileType 如果是大写转为小写
        fileType = fileType.toLowerCase()
        this.fileTypeMIME = MIMEFormat([fileType])[0]
        let previewTypes = [
          'application/pdf',
          'image/jpeg',
          'image/png',
        ...
        if (previewTypes.includes(this.fileTypeMIME)) {
          this.previewDevice(item, this.fileTypeMIME)
          return false
        }
      }
